...
window.AI_UPDATES = [
  { date: "2026-06-29", category: "ops", title: "일일 운영 — 소식 11건 · 신규 갭(커뮤니티·뉴스레터로 계속 배우기) · 보강(Claude·Cowork·Code 차이)", summary: "A) 실전·심화 위주 소식 11건 — 영상3(멀티에이전트 에이전틱 코딩[IndyDevDan]·코딩 에이전트를 작동시키는 엔지니어링 실천법[Simon Willison·The Pragmatic Engineer]·MCP 서버 밑바닥부터 만들기[Mehul Mohan]) · GitHub2(VoltAgent/awesome-claude-code-subagents·disler/claude-code-hooks-multi-agent-observability) · 해외 1차2(OpenAI Codex ‘에이전트 스킬’ 공식문서·Google 개발자블로그 ‘에이전트 스킬로 지식격차 메우기’) · 기술블로그1(Simon Willison ‘바이브코딩과 에이전틱 엔지니어링이 가까워진다’) · 논문1(arXiv 2606.17799 — 코딩 벤치마크는 에이전틱 SW엔지니어링과 어긋나 있다) · 국내2(머니투데이 통신3사 자율형 AI 에이전트·ZDNet 젠스파크 AI 워크스페이스). 영상은 YouTube oembed로 제목·채널 검증, 해외·GitHub·블로그·논문은 web_fetch 본문 확인, 국내는 web_fetch로 pubDate(06-28~29) 실링크 확인. 06-25~28 자동분과 중복 제외(중복 후보 1건[Codex 입문 0TitiOk7hbI] 발견·다른 실링크로 교체). ※날짜=KST 2026-06-29. B) 백로그 pending-fix 없음. 오늘=기초·활용일(일자 29 → 끝자리 9). 신규 갭 start#community(커뮤니티·뉴스레터로 계속 배우기 — ‘신문 구독’ 비유·활용상황 미니그리드·정보 다이어트 SVG 흐름도·구독/팔로우/실습 steps·요약위임 프롬프트·io 2개·증상/원인/처방표·체크리스트; The Batch·TLDR AI·GeekNews·Hacker News·Anthropic News 실링크; #community 삽입·TOC·main.js 동기화, 점수 93). 보강 claude#diff(Claude·Cowork·Code 차이 — ‘내게 맞는 것 고르기’ steps+프롬프트·io 2개·문제해결표·claude.ai/Cowork/Code docs/요금제 공식 링크, 키보드 이모지 제거 → 36→93). B5 검토 에이전트: 두 섹션 5항목(사실·이해·중복·톤·링크) 모두 PASS, 외부 9개 링크 전수 실재·정합 확인, FIX/HOLD 없음. 채점 평균 69.4→70.4·중앙 71→79(n81→82, 0–39 8→7·80–100 35→37, 하락 섹션 0).", source: "운영봇", level: "입문" },
  ...
  { date: "2026-06-28", category: "ops", title: "일일 운영 — 소식 11건 · 신규 갭(프라이버시·데이터 보호) · 보강(가입·접속하기)", summary: "A) 실전·심화 위주 소식 11건 — 영상3(효과적인 Claude Code 에이전트 만들기[Nate Herk]·Claude Code 첫 AI 에이전트[Teacher’s Tech]·Claude 완전 튜토리얼 기초→에이전틱[The AI Productivity Coach]) · GitHub2(VoltAgent/awesome-agent-skills·awesome-ai-agent-papers) · 해외 1차2(OpenAI Codex×MCP 공식문서·Anthropic ‘Claude Code 40만 세션 분석’) · 기술블로그1(OpenAI Developers: 스킬+GitHub Actions로 OSS 유지보수 가속) · 논문1(arXiv 2602.05892 ContextBench) · 국내2(KB증권 금융권 최초 MCP 허브·클로드 코드 개발환경[시사오늘]·‘도입보다 인식 변화’ CEO AX 교육[IT조선]). 영상은 YouTube oembed로 제목·채널 검증, 해외·GitHub·블로그·논문은 web_fetch 본문 확인, 국내는 NaverSearch 당일 pubDate(06-26~28)로 실링크 확인. 06-25~27 자동분과 중복 제외. ※날짜=KST 2026-06-28. B) 백로그 pending-fix 없음. 오늘=기초·활용일(일자 28 → 끝자리 8). 신규 갭 start#privacy(프라이버시·데이터 보호 — ‘돌려받을 수 없는 엽서’ 비유·민감정보 미니그리드·데이터 흐름 SVG·비식별화 프롬프트·학습 끄기/임시·시크릿 채팅 steps·io 2개·증상/원인/처방표·체크리스트; OpenAI 데이터제어FAQ·Anthropic 학습여부·Gemini 데이터·PIPC AI 프라이버시 리스크 관리 모델 실링크; #privacy 삽입·TOC·main.js 동기화, 점수 93). 보강 start#signup(가입·접속 — 가입 3단계 SVG·시작 프롬프트·첫질문/인증 io·문제표·체크리스트·Claude/ChatGPT 시작 공식링크 → 36→93). B5 검토 에이전트: 두 섹션 5항목(사실·이해·중복·톤·링크) 모두 PASS, 외부 6개 링크 전수 실재·정합 확인, FIX/HOLD 없음. 채점 평균 68.4→69.4·중앙 71(n80→81, 0–39 9→8·80–100 33→35, 하락 섹션 0).", source: "운영봇", level: "입문" },
  ...
  { date: "2026-06-27", category: "ops", title: "일일 운영 — 소식 11건 · 신규 갭(AI 편향·윤리) · 보강(ChatGPT 요금제)", summary: "A) 실전·심화 위주 소식 11건 — 영상3(2026 Claude Code 워크플로[Ray Amjad]·Claude 스킬 만들기·공유[Kevin Stratvert]·Cowork 제로투프로[AI Foundations]) · GitHub2(openai/codex 8.7만★·shanraisshan/claude-code-best-practice) · 해외 1차2(앤트로픽 서울 오피스·韓 파트너십[네이버 Claude Code 전사 도입·삼성SDS·LG CNS]·OpenAI ChatGPT 메모리 ‘Dreaming’) · 기술블로그1(Simon Willison: 서브에이전트 패턴) · 논문1(arXiv 2606.24937 에이전틱 AI 안내서) · 국내2(삼성SDS·LG CNS·SK AX 업무전환·협업툴 ‘사스포칼립스’ AX). 영상은 YouTube oembed로 제목·채널 검증, 해외·GitHub·블로그·논문은 web_fetch 본문 확인, 국내는 NaverSearch 당일 pubDate(06-26~27)로 실링크 확인. 06-24~26 자동분과 중복 제외. ※날짜=KST 2026-06-27. B) 백로그 pending-fix 없음. 오늘=기초·활용일(일자 27 → 끝자리 7). 신규 갭 start#ethics(AI 편향·윤리 — ‘거울’ 비유·편향 상황 미니그리드·데이터→학습→결과→사람점검 SVG 흐름도·자가점검 프롬프트·io 2개·증상/원인/처방표·체크리스트·KISDI/UNESCO/NIST SP1270/YTN 실링크; #ethics 삽입·TOC·main.js 동기화, 점수 93). 보강 chatgpt#price(요금제 — 활용상황 미니그리드·요금 사다리 SVG·구독/해지 steps·io 2개·문제표·공식 해지/이중결제 링크 → 36→93). B5 검토 에이전트: price PASS, ethics FIX 1건 반영(NIST 링크를 RMF 랜딩 → 편향 3종이 실제 실린 SP 1270 발표 페이지로 교체). 채점 평균 67.3→68.4·중앙 71(n79→80, 0–39 10→9·80–100 31→33, 하락 섹션 0). HOLD/롤백 없음.", source: "운영봇", level: "입문" },
  ...
  { date: "2026-06-26", category: "ops", title: "일일 운영 — 소식 11건 · 신규 갭(법무·인사·재무) · 보강(활용 사다리)", summary: "A) 실전·심화 소식 11건 — 영상3(Claude Cowork 입문[Wisdom Speaks]·Claude Code 비개발자 입문[Futurepedia]·Codex 풀코스[David Ondrej]) · GitHub2(anthropics/skills 14.9만★·lastmile-ai/mcp-agent) · 해외 1차2(앤트로픽 ‘클로드 태그’·OpenAI Codex CLI 공식문서) · 기술블로그1(Developers Digest: Claude Code 에이전트 팀·서브에이전트·MCP) · 논문1(arXiv 2606.22678 RigorBench) · 국내2(AI타임스 ‘모델이 하네스를 먹어치울 것’+클로드 태그·현대글로비스 바이브코딩 부트캠프). 영상은 YouTube oembed로 제목·채널 검증, 해외·GitHub·블로그·논문은 web_fetch 본문 확인, 국내는 NaverSearch 당일 pubDate(06-25~26)로 실링크 확인. 06-24~25 자동분과 중복 제외. ※날짜=KST 2026-06-26. B) 백로그 pending-fix 없음. 신규 갭 usecases#backoffice(법무·인사·재무 — 계약 검토·채용·회계 문서; 앤트로픽 법무/금융 솔루션·Hector Garcia CPA 영상 실링크 근거; #14 삽입·TOC·main.js 동기화, 점수 100). 보강 advanced#ladder(활용 사다리 — 인라인 SVG 4단계 도식·‘한 칸 올라가기’ io 2개·함정표·체크리스트·MCP/스킬 링크리스트 → 29→100). B5 검토 에이전트: 두 섹션 5항목(사실·이해·중복·톤·링크) 모두 PASS, 외부 5개·내부 9개 링크 전수 실재 확인, FIX/HOLD 없음. 채점 평균 65.4→66.7·중앙 64(n77→78, 0–39 11→10·80–100 26→28, 하락 섹션 0).", source: "운영봇", level: "입문" },
